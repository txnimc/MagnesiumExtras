...
import toni.embeddiumextras.mixins.impl.borderless.accessors.MainWindowAccessor;
...
@Mod.EventBusSubscriber(modid = EmbeddiumExtras.ID, bus = Mod.EventBusSubscriber.Bus.MOD, value = Dist.CLIENT)
public class EmbyConfig {
    ...
    public static void load() {
        if (isLoaded()) return;
        EmbeddiumExtras.LOGGER.warn(IT, "Loading Embeddium++Config");
    ...
    public static void updateCache(ModConfigEvent ignored) {
        EmbeddiumExtras.LOGGER.info(IT,"Updating config cache");
        ...
        EmbeddiumExtras.LOGGER.info(IT,"Cache updated successfully");
    }
